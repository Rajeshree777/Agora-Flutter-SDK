...
float filtered_bg_simple2( sampler2D mask_tex, vec2 uv )
{
	float bg1 = 1. - texture( mask_tex, uv ).x;

	vec2 o = 1.5/vec2(textureSize(mask_tex,0));
	float bg2 = texture( mask_tex, uv + vec2(o.x,0.) ).x;
	float bg3 = texture( mask_tex, uv - vec2(o.x,0.) ).x;
	float bg4 = texture( mask_tex, uv + vec2(0.,o.y) ).x;
	float bg5 = texture( mask_tex, uv - vec2(0.,o.y) ).x;

	return 1.-0.2*(bg1+bg2+bg3+bg4+bg5);
}
...
void main()
{
	float darkening = mix( BG_DARKENING, 1., filtered_bg_simple2( glfx_BG_MASK, var_bgmask_uv ) );
	float mask = filtered_bg_simple( glfx_BG_MASK, var_bgmask_uv );
	F = darkening*mix( texture(glfx_BACKGROUND,var_uv), textureBicubicLod( glfx_BACKGROUND, var_uv, 3. ), pow(mask,0.25) );
	//F = vec4(darkening);
}

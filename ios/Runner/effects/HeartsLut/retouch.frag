...
uniform sampler2D selection_tex, lookupTexEyes, lookupTexTeeth, tex_highlight, glfx_BACKGROUND;
...
void main()
{
    ...
    res = vec4(one * res.rgb, 1.0);
    res = res + vec4( texture( tex_highlight, var_uv ).xyz, 0. );

    res.xyz = softlight_blend_1ch( res.xyz, texture( tex_makeup, var_uv ).xyz );
    F = res;

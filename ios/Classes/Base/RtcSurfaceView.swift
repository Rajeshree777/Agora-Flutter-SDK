...
class RtcSurfaceView: UIView {
    ...
    override func layoutSubviews() {
        print("run when UIView appears on screen banuba")
    ...
    private func setupVideoCanvas(_ engine: AgoraRtcEngineKit) {
        ...
        surface.frame = CGRect(origin: CGPoint(x: 0, y: 0), size: UIScreen.main.bounds.size)
        print("Setup Video Canvas Banuba \(self.frame.size) === \(surface.frame)")
        canvas.view = surface
        if canvas.uid == 0 {
            engine.setupLocalVideo(canvas)
